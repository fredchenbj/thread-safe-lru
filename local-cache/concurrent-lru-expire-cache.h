...
/**
 * ConcurrentLRUExpireCache is a thread-safe hashtable with a limited size. When
...
struct ConcurrentLRUExpireCache {
private:
    /**
     * The LRU list node.
     *
    ...
    struct ListNode {
        ListNode()
                : m_timestamp(0),m_prev(OutOfListMarker), m_next(nullptr) {}
...
public:
    ...
    private:
        friend struct ConcurrentLRUExpireCache;
     ...
     */
    explicit ConcurrentLRUExpireCache(size_t maxSize,int64_t m_ttlSecond);

    ConcurrentLRUExpireCache(const ConcurrentLRUExpireCache &other) = delete;

    ConcurrentLRUExpireCache &operator=(const ConcurrentLRUExpireCache &) = delete;

    ~ConcurrentLRUExpireCache() {
        clear();
    }
...
private:
     ...
     */
    void pushFront(ListNode *node);//链表的头部是最近访问的节点
     ...
     * @param node
     */
    void evictByExpire(HashMapConstAccessor &hashAccessor);
...
typename ConcurrentLRUExpireCache<TKey, TValue, THash>::ListNode *const
        ConcurrentLRUExpireCache<TKey, TValue, THash>::OutOfListMarker = (ListNode * ) - 1;

template<class TKey, class TValue, class THash>
ConcurrentLRUExpireCache<TKey, TValue, THash>::
ConcurrentLRUExpireCache(size_t maxSize, int64_t ttlSecond)
...
template<class TKey, class TValue, class THash>
bool ConcurrentLRUExpireCache<TKey, TValue, THash>::
find(ConstAccessor &ac, const TKey &key) {
    ...
    // 判断是否过期，如果过期则delink和erase
    std::time_t timestamp = std::time(NULL);
    timestamp -= m_ttlSecond;
    // 如果node写入的时间戳+过期时间长度<当前时间，则node过期，要清除
    if(node->m_timestamp < timestamp) {
        evictByExpire(hashAccessor);
        return false;
    }
...
insert(const TKey &key, const TValue &value) {
    // Insert into the CHM
    std::time_t timestamp = std::time(NULL);
    ListNode *node = new ListNode(key,timestamp);//链表用来存放key，m_map用来存放包含key和value组成的一个结构的值，如281行
...
template<class TKey, class TValue, class THash>
void ConcurrentLRUExpireCache<TKey, TValue, THash>::
clear() {
    m_map.clear();
...
template<class TKey, class TValue, class THash>
inline void ConcurrentLRUExpireCache<TKey, TValue, THash>::
...
evict() {
    ...
    // 从hash表中删除
    bool erase = m_map.erase(hashAccessor);
    if(erase) {
        delete moribund;
    }
}
...
template<class TKey, class TValue, class THash>
void ConcurrentLRUExpireCache<TKey, TValue, THash>::
evictByExpire(HashMapConstAccessor &hashAccessor) {
    std::unique_lock <ListMutex> lock(m_listMutex);
    ListNode *node = hashAccessor->second.m_listNode;
    // 从链表中删除
    if (node->isInList()) {
